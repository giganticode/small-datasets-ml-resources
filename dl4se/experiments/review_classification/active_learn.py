...
from dl4se.datasets.review_classification import Dataset
...
def main(config, results):
    ...
    train_valid_df, test_df = train_test_split(ds.df, test_size=0.2, shuffle=True,
                                               stratify=ds.df.label_id, random_state=config.seed)

    results = active_learn(config, model_config, tokenizer, results, ds.label_names, test_df, train_valid_df,
                           backtrans_dfs, ds.get_dataloader, run_configs, balance=True)
    return results
